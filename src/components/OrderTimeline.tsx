  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
        <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-6 flex items-center justify-between">
        ...
        {/* Failed Alert Banner */}
        {isFailed && (
          <div className="mb-6 p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 text-rose-800 dark:text-rose-200 flex items-start gap-3">
        ...
        {isRescheduled && (
          <div className="mb-6 p-4 rounded-2xl bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 text-purple-800 dark:text-purple-200 flex items-start gap-3">
                ...
                <div key={step.key} className="flex-1 flex flex-col items-center relative group">
                  ...
                  {idx < ORDER_STEPS.length - 1 && (
                    <div
                      className={`absolute top-4 left-1/2 right-[-50%] h-1 z-0 transition-colors duration-500 ${
                        idx < currentIndex ? "bg-indigo-600 dark:bg-indigo-500" : "bg-slate-200 dark:bg-slate-800"
                      }`}
                    />
                  )}
                  ...
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs z-10 transition-all shadow-sm ${
                      isCompleted
                        ...
                        : isCurrent
                        ? "bg-indigo-600 dark:bg-indigo-500 text-white ring-4 ring-indigo-200 dark:ring-indigo-900/60 animate-pulse"
                  ...
                  <span
                    className={`mt-2 text-xs font-medium text-center transition ${
                      isCompleted || isCurrent ? "text-slate-900 dark:text-white font-semibold" : "text-slate-400 dark:text-slate-500"
                    }`}
                  >
                    {step.label}
                  </span>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
        <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">{history.length} events logged</span>
        </div>
          ...
          {history.map((event, idx) => {
            ...
            return (
              <div key={event.id || idx} className="relative group">
                {/* Timeline node dot */}
                <div
                  className={`absolute -left-6 top-1 w-3.5 h-3.5 rounded-full border-2 border-white dark:border-slate-900 shadow-sm ${
                    isLatest
                      ? event.status === "FAILED"
                ...
                <div className="bg-slate-50 dark:bg-slate-800/60 hover:bg-slate-100/80 dark:hover:bg-slate-800 transition p-4 rounded-2xl border border-slate-200/80 dark:border-slate-700/80">
                  ...
                  {/* Actor Details */}
                  <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 mt-1 mb-2">
                    <span className="inline-flex items-center gap-1 bg-white dark:bg-slate-900 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 font-medium text-slate-800 dark:text-slate-200">
                      {event.actorRole === "ADMIN" && <Shield className="w-3 h-3 text-indigo-600 dark:text-indigo-400" />}
                      {event.actorRole === "AGENT" && <Truck className="w-3 h-3 text-amber-600 dark:text-amber-400" />}
                      {event.actorRole === "CUSTOMER" && <User className="w-3 h-3 text-blue-600 dark:text-blue-400" />}
                  ...
                  {/* Notes / Explanation */}
                  {event.notes && (
                    <p className="text-xs text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-700/60 leading-relaxed">
                      {event.notes}
                    </p>
                  )}
                </div>
