...
export default function TrackOrderPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <div className="text-slate-600 dark:text-slate-400 text-sm font-medium">Fetching real-time tracking data...</div>
      </div>
    );
  }
  ...
  if (error || !order) {
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-semibold hover:bg-indigo-700 transition shadow"
        >
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          {order.assignedAgent && (
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 transition-colors">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-bold text-sm">
                    {order.assignedAgent.name.split(" ").map((n: string) => n[0]).join("")}
                  </div>
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white text-sm">{order.assignedAgent.name}</div>
                ...
                <div className="text-right">
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 transition-colors">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
            ...
            <div className="text-xs space-y-2">
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>Contents:</span>
                <span className="font-semibold text-slate-900 dark:text-white">{order.itemDescription}</span>
              </div>
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>Actual Weight:</span>
                <span className="font-mono text-slate-900 dark:text-white">{order.actualWeightKg} kg</span>
              </div>
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>Volumetric Weight (L×B×H ÷ 5000):</span>
                <span className="font-mono text-indigo-600 dark:text-indigo-400 font-semibold">{order.volumetricWeightKg} kg</span>
              </div>
              <div className="flex justify-between font-bold text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800 p-2 rounded-xl border border-slate-200 dark:border-slate-700">
                <span className="flex items-center gap-1">
                  <Sparkles className="w-3.5 h-3.5 text-amber-500" /> Billable Weight:
                </span>
                <span className="font-mono text-indigo-700 dark:text-indigo-400">{order.billableWeightKg} kg</span>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div>
                ...
                <span className="text-xl font-black text-slate-900 dark:text-white">${order.totalAmount.toFixed(2)}</span>
              </div>
