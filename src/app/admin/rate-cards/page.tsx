    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Dynamic Rate Cards</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {loading ? (
          <div className="col-span-2 py-16 text-center text-xs text-slate-500 dark:text-slate-400">Loading rate cards...</div>
        ) : (
          rateCards.map((card) => (
            <div
              key={card.id}
            ...
            >
              <div className="flex items-start justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-slate-900 dark:text-white text-base">{card.name}</h3>
                    <span
                ...
                <button
                  ...
                  className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-950 text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-300 transition"
                >
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="bg-slate-50 dark:bg-slate-800/70 p-3 rounded-2xl border border-slate-100 dark:border-slate-700/80">
                  <span className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold block">Base Pricing</span>
                  <div className="text-base font-bold text-slate-900 dark:text-white mt-0.5">${card.baseRate.toFixed(2)}</div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">Up to {card.baseWeightKg} kg</div>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800/70 p-3 rounded-2xl border border-slate-100 dark:border-slate-700/80">
                  <span className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold block">Incremental Rate</span>
                  <div className="text-base font-bold text-indigo-600 dark:text-indigo-400 mt-0.5">${card.perKgRate.toFixed(2)} / kg</div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">Beyond base weight</div>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800/70 p-3 rounded-2xl border border-slate-100 dark:border-slate-700/80 col-span-2">
      ...
      {editingCard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 dark:border-slate-800 transition-colors">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="font-bold text-slate-900 dark:text-white text-lg">Edit Rate Card</h3>
              <button onClick={() => setEditingCard(null)} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleUpdate} className="space-y-4 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Rate Card Name</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Base Weight (kg)</label>
                  <input
                    type="number"
                    step="0.1"
                    required
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Base Rate ($)</label>
                  <input
                    type="number"
                    step="0.5"
                    required
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Per Kg Rate ($)</label>
                  <input
                    type="number"
                    step="0.5"
                    required
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">COD Fixed ($)</label>
                  <input
                    type="number"
                    step="1"
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">COD % Surcharge</label>
                  <input
                    type="number"
                    step="0.1"
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Min COD Fee ($)</label>
                  <input
                    type="number"
                    step="1"
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="isActiveToggle" className="font-semibold text-slate-800 dark:text-slate-200">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 dark:border-slate-800 transition-colors">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="font-bold text-slate-900 dark:text-white text-lg">Create New Rate Card</h3>
              <button onClick={() => setShowCreateModal(false)} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Order Type</label>
                  <select
                    value={newOrderType}
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Zone Scope</label>
                  <select
                    value={newZoneScope}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">COD %</label>
                  <input
                    type="number"
                    step="0.1"
                    value={newCodPercent}
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Min COD ($)</label>
                  <input
                    type="number"
                    step="1"
                    value={newMinCod}
